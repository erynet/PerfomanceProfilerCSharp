        ...
        public CaptureProcess(Process process, CaptureConfig config, CaptureInterface captureInterface)
        {
            ...
            try
            {
                ...
                RemoteHooking.Inject(
                    ...
                    typeof(CaptureInterface).Assembly.Location,//"Direct3DHookLib.dll", // 32-bit version (the same because AnyCPU) could use different assembly that links to 32-bit C++ helper dll
                    typeof(CaptureInterface).Assembly.Location, //"Direct3DHookLib.dll", // 64-bit version (the same because AnyCPU) could use different assembly that links to 64-bit C++ helper dll
