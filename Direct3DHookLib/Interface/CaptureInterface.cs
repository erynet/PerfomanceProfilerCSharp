...
namespace Direct3DHookLib.Interface
{
    ...
    [Serializable]
    public delegate void ReportPropertyReceivedEvent(ReportPropertyReceivedEventArgs message);
    [Serializable]
    public delegate void ReportFpsReceivedEvent(ReportFpsReceivedEventArgs message);
    [Serializable]
    public delegate void ReportFpsTimecodeReceivedEvent(ReportFpsTimecodeReceivedEventArgs message);
    [Serializable]
    ...
    public class CaptureInterface : MarshalByRefObject
    {
        ...
        public event ReportPropertyReceivedEvent RemoteReportProperty;

        public event ReportFpsReceivedEvent RemoteReportFps;

        public event ReportFpsTimecodeReceivedEvent RemoteReportFpsTimecode;
        ...
        #region Still image Direct3DHookLib
        ...
        public void ReportFpsTimecode(Int64 timecode) 
        {
#if DEBUG
            //System.Console.WriteLine("### ReportFpsTimecode / Timecode :" + timecode);
#endif
            SafeInvokeReportFpsTimecodeReceived(new ReportFpsTimecodeReceivedEventArgs(timecode));
        }
        ...
        private void SafeInvokeReportPropertyReceived(ReportPropertyReceivedEventArgs eventArgs)
        {

            //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived / Pid : " + eventArgs.Pid + ", dxVersion : " + eventArgs.DxVersion);
            if (RemoteReportProperty == null)
                return;          //No Listeners
            //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived #1");
            ReportPropertyReceivedEvent listener = null;
            Delegate[] dels = RemoteReportProperty.GetInvocationList();
            //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived / dels.Length : " + dels.Length);
            foreach (Delegate del in dels)
            {
                try
                {
                    //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived #2");
                    listener = (ReportPropertyReceivedEvent)del;
                    listener.Invoke(eventArgs);
                    //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived #2-1");
                }
                catch (Exception)
                {

                    //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived #3 : " + e.Message);
                    RemoteReportProperty -= listener;
                    //System.Console.WriteLine("  ##### SafeInvokeReportPropertyReceived #3-1");
                }
            }
        }

        private void SafeInvokeReportFpsTimecodeReceived(ReportFpsTimecodeReceivedEventArgs eventArgs)
        {
            if (RemoteReportFpsTimecode == null)
                return;          //No Listeners
            ReportFpsTimecodeReceivedEvent listener = null;
            Delegate[] dels = RemoteReportFpsTimecode.GetInvocationList();

            foreach (Delegate del in dels)
            {
                try
                {
                    listener = (ReportFpsTimecodeReceivedEvent)del;
                    listener.Invoke(eventArgs);
                }
                catch (Exception)
                {
                    RemoteReportFpsTimecode -= listener;
                }
            }
        }


        private void SafeInvokeReportFpsReceived(ReportFpsReceivedEventArgs eventArgs)
        {

            if (RemoteReportFps == null)
                return;          //No Listeners
            ReportFpsReceivedEvent listener = null;
            Delegate[] dels = RemoteReportFps.GetInvocationList();
            foreach (Delegate del in dels)
            {
                try
                {
                    listener = (ReportFpsReceivedEvent)del;
                    listener.Invoke(eventArgs);
                    
                }
                catch (Exception)
                {
                    RemoteReportFps -= listener;
                }
            }
        }
